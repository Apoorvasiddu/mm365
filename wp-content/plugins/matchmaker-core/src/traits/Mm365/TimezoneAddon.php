...
trait TimezoneAddon
{
     ...
     * 
     */
    public function read_timezone(){
        $tz         =  $_POST['timezone'];
        $offset     =  $_POST['offset'];
        $dst        =  $_POST['dst'];

        if(!isset($_COOKIE['user_timezone'])){
         setcookie('user_timezone', $this->detect_timezone_id($offset,$dst), time() + 3600, '/',"");
        }
        echo $this->detect_timezone_id($offset,$dst);
        die();
     }
    ...
    public static function detect_timezone_id($offset, $dst, $multiple = FALSE, $default = 'UTC')
    {
        ...
        $timezones = self::mm365_wp_timezone_choice(false,false);
     ...
     * 
     */
    public static function mm365_wp_timezone_choice($selected = NULL,$ret = 'list'){
        ...
        $additional = '';
        foreach ($common_tz as $key => $value) {
            ($selected == $value) ? $sel = 'selected': $sel = '';
            $additional .=  '<option '.$sel.' value="'.$value.'">'.$key.'</option>';
        }
        //$additional .=  '';

        if($ret == 'list'){
          echo $additional;
        }else return $common_tz;

    }

    public static function get_timezone_commonname($timezone){
        //$timezones = self::mm365_wp_timezone_choice(false,false);
        //return array_search($timezone,$timezones);
        return $timezone;
    }
     ...
     * 
     */
     public function convert_time( $time_to_convert = null, $start_timezone_string = "UTC", $end_timezone_string = "UTC", $date_format = 'm/d/Y' ) {
	
        // We require a start time
        if( empty( $time_to_convert ) ){
            return false;
        }
        if($start_timezone_string == ''){ $start_timezone_string = 'UTC'; }
        $start_time = new \DateTimeZone($start_timezone_string);
        $datetime = new \DateTime(date('Y-m-d H:i:s',$time_to_convert));
        $datetime->setTimezone($start_time);
        //echo $datetime->format('Y-m-d H:i:s') . "<br/>";
        if($end_timezone_string == ''){ $end_timezone_string = 'UTC'; }
        $end_time = new \DateTimeZone($end_timezone_string);
        $datetime->setTimezone($end_time);
     ...
     * 
     */
    function make_timestamp($date_and_time, $timezone){
